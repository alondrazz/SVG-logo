...
function generateSVG(answers) {
    ...
    if (shapes.includes('circle')) {
        ...
        svg += `<text x="150" y="110" font-size="20" text-anchor="middle" fill="${textColor}">${text}</text>`
    }
        // Add the square shape
    if (shapes.includes('square')) {
        svg += `<rect x="90" y="70" width="120" height="120" fill="${backgroundColor}" />`;
        svg += `<text x="150" y="140" font-size="40" text-anchor="middle" fill="${textColor}">${text}</text>`
    }
